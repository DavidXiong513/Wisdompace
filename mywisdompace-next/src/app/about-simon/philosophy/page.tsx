...
const QUOTES = [
  ...
  { text: "真正的自由不是想做什么做什么，是在每个不得不做的角色里，依然能做自己。", category: "借假修真" },
          ...
          <div className="mx-auto grid max-w-4xl gap-5 lg:grid-cols-3">
            {CORE_IDEOLOGY.map((item) => (
              <div key={item.title} className="as-card p-5 text-center">
                ...
                <h3 className="mt-4 as-serif text-xl font-bold text-[var(--as-primary-700)]">
                  {item.title}
                </h3>
                <p className="mt-1 text-xs text-[var(--as-gray-400)]">
                  {item.subtitle}
                </p>
            ...
            {METHODOLOGIES.map((m) => (
              <div key={m.title} className="as-card p-5">
                ...
                <div className="mt-3 rounded-lg bg-[var(--as-primary-50)] p-3">
                  ...
                  <p className="mt-1 text-sm text-[var(--as-primary-700)]">
                    {m.example}
                  </p>
                </div>
              </div>
          ...
          <div className="columns-1 gap-3 sm:columns-2 lg:columns-3">
            {QUOTES.map((q, i) => (
              ...
              >
                ...
                <p className="mt-2 text-xs text-[var(--as-gray-400)]">
                  — {q.category}
                </p>
              </div>
            ))}
