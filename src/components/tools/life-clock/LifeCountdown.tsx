...
const TimeUnit = ({ value, label }: { value: number; label: string }) => (
  <div className="group flex flex-col items-center">
    <div className="relative">
      <div className="absolute -inset-2 rounded-lg bg-[#C9A15A]/5 opacity-0 blur-sm transition-opacity group-hover:opacity-100" />
      <div className="relative text-3xl font-bold text-[#B45309] tabular-nums sm:text-4xl lg:text-5xl">
        {String(value).padStart(2, '0')}
      </div>
    </div>
    <div className="mt-2 text-[10px] font-bold tracking-[0.25em] text-[#C9A15A] uppercase">
      {label}
    </div>
  </div>
);
...
export function LifeCountdown({ result }: Props) {
  ...
  const endTime = useMemo(
    () => new Date(Date.now() + result.remainingYears * 365.25 * 24 * 60 * 60 * 1000),
  ...
  const timeLeft = useMemo(() => {
    if (!endTime || !now) return { years: 0, months: 0, days: 0, hours: 0, minutes: 0, seconds: 0 };
  ...
  return (
    <div className="rounded-2xl border-2 border-[#C9A15A]/20 bg-white p-5 shadow-[0_8px_30px_rgb(201,161,90,0.08)] sm:p-10">
      <div className="mb-8 flex items-center justify-center gap-3">
        ...
        <p className="text-center text-xs font-bold tracking-[0.3em] text-[#C9A15A] uppercase">
          生命余晖剩余刻度
        </p>
      ...
      <div className="mt-10 border-t border-[#C9A15A]/10 pt-8 text-center">
        <p className="text-sm font-medium text-[#8A7E6A]">
          预计总寿命：
          <span className="text-lg font-bold text-[#B45309]">{result.expectedLifespan}</span> 岁
        </p>
      </div>
    </div>
  );
