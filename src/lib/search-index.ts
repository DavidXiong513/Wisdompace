...
function buildDocs(): Doc[] {
  ...
  for (const { code, data } of languages) {
    const chapterContent = (data as any).chapterContent || {};
    const chapterLabels = (data as any).chapter || {};

    // 1. Read Instructions
    const riData = chapterContent["read-instructions"] || {};
    const riTitle = chapterLabels.readInstructions || (code.startsWith("zh") ? "阅读说明" : "Reader's Guide");
    
    // Index full page
    const riFullText = Object.values(riData).map((s: any) => 
...
export function searchAll(query: string, lng = "zh-CN", limit = 12): SearchHit[] {
  ...
  const hits = DOCS.filter(d => d.lng === lng || (lng.startsWith("en") && d.lng === "en") || (lng.startsWith("zh") && d.lng.startsWith("zh")))
    .map((d) => {
