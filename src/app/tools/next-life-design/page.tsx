...
function generateReport(scores: AttributeScores) {
  ...
  const valueReflection = destiny.isBalanced
    ...
    : `你把最多的福德给了「${topNames.join('、')}」，这不是随意的选择——它照见了你今生最深处的价值排序。你舍得在这些领域投入，说明你潜意识里认定它们是"值得的"。而「${bottomNames.join('、')}」被你放在了末位，也自然反映了你当下的取舍——在你心里，有比它们更在乎的事排在了前面。50 分的分配，就是你灵魂的一份供词。`;
  ...
  const reminder = destiny.isBalanced
    ...
    : `50 分终究是有限资源。你在「${topNames.join('、')}」上的慷慨，是对自己灵魂需求的诚实；而在「${bottomNames.join('、')}」上的克制，也恰恰照见了你的真实优先级——你给得少的地方，不一定是不在乎，只是在你的排序里，有更重要的事落在了前面。来生不会完美——50 分覆盖不了 80 分的人生，缺口才是功课所在。但这份配置会帮你更清楚地看见：你究竟想要怎样的人生。`;
